                        ...
                        <tr data-entry-id="{{ $vehicleModel->id }}">
                            ...
                            <td>
                                @can('vehicle_model_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.vehicle-models.show', $vehicleModel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('vehicle_model_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.vehicle-models.edit', $vehicleModel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('vehicle_model_delete')
                                    <form action="{{ route('admin.vehicle-models.destroy', $vehicleModel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
