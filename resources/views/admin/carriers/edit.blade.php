    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.carriers.update", [$carrier->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $carrier->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vehicle_model') ? 'is-invalid' : '' }}" name="vehicle_model_id" id="vehicle_model_id" required>
                    @foreach($vehicle_models as $id => $entry)
                        <option value="{{ $id }}" {{ (old('vehicle_model_id') ? old('vehicle_model_id') : $carrier->vehicle_model->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('delivery_type') ? 'is-invalid' : '' }}" name="delivery_type_id" id="delivery_type_id" required>
                    @foreach($delivery_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('delivery_type_id') ? old('delivery_type_id') : $carrier->delivery_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vehicle_type') ? 'is-invalid' : '' }}" name="vehicle_type_id" id="vehicle_type_id" required>
                    @foreach($vehicle_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('vehicle_type_id') ? old('vehicle_type_id') : $carrier->vehicle_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vehicle_number') ? 'is-invalid' : '' }}" type="text" name="vehicle_number" id="vehicle_number" value="{{ old('vehicle_number', $carrier->vehicle_number) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trailer_size') ? 'is-invalid' : '' }}" name="trailer_size_id" id="trailer_size_id" required>
                    @foreach($trailer_sizes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('trailer_size_id') ? old('trailer_size_id') : $carrier->trailer_size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
