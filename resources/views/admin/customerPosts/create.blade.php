...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.customerPost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-posts.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('from_city') ? 'is-invalid' : '' }}" type="text" name="from_city" id="from_city" value="{{ old('from_city', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('to_city') ? 'is-invalid' : '' }}" type="text" name="to_city" id="to_city" value="{{ old('to_city', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('load_type') ? 'is-invalid' : '' }}" name="load_type_id" id="load_type_id" required>
                    @foreach($load_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('load_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
