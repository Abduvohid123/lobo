...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.expensesCustomerPost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.expenses-customer-posts.update", [$expensesCustomerPost->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $expensesCustomerPost->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('spent_coins') ? 'is-invalid' : '' }}" type="number" name="spent_coins" id="spent_coins" value="{{ old('spent_coins', $expensesCustomerPost->spent_coins) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('for') ? 'is-invalid' : '' }}" type="text" name="for" id="for" value="{{ old('for', $expensesCustomerPost->for) }}" required>
                @if($errors->has('for'))
                    <div class="invalid-feedback">
                        {{ $errors->first('for') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer_post') ? 'is-invalid' : '' }}" name="customer_post_id" id="customer_post_id" required>
                    @foreach($customer_posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_post_id') ? old('customer_post_id') : $expensesCustomerPost->customer_post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
