...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.expensesDeclarantPost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.expenses-declarant-posts.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('declarant_post') ? 'is-invalid' : '' }}" name="declarant_post_id" id="declarant_post_id" required>
                    @foreach($declarant_posts as $id => $entry)
                        <option value="{{ $id }}" {{ old('declarant_post_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
