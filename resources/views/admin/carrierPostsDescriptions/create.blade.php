...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.carrierPostsDescription.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.carrier-posts-descriptions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carrier_post') ? 'is-invalid' : '' }}" name="carrier_post_id" id="carrier_post_id" required>
                    @foreach($carrier_posts as $id => $entry)
                        <option value="{{ $id }}" {{ old('carrier_post_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
